...
**Windows** — **PowerShell을 그대로 쓰면 된다.** 이 문서의 모든 명령은 PowerShell판을 함께 제공한다. 단 세 가지만 알아두자.

1. **`bootstrap-dev.sh`는 bash 스크립트라 PowerShell에서 직접 못 돌린다.** Git Bash를 경유한다(아래 단계 5에 명령이 있다). Git for Windows를 설치하면 함께 깔린다.
2. **`bash`를 그냥 치면 WSL이 잡힌다.** Windows에는 `C:\WINDOWS\system32\bash.exe`(WSL)가 있어서 `bash script.sh`가 Git Bash가 아니라 WSL로 간다. WSL에서는 `docker`가 안 보일 수 있으므로, **Git Bash의 전체 경로**(`C:\Program Files\Git\bin\bash.exe`)로 부른다.
3. **PowerShell에서 `curl`은 `curl`이 아니다.** `Invoke-WebRequest`의 별칭이라 `-d`·`-F` 같은 플래그를 이해하지 못한다. 반드시 **`curl.exe`** 로 쓴다(3장 예제는 그렇게 되어 있다).

커밋 훅은 걱정하지 않아도 된다 — git이 알아서 자기 bash로 실행한다.
...
터미널을 열고 저장소 루트에서 시작한다. **Windows는 PowerShell, macOS·Linux는 아무 셸이나** 쓰면 된다. 셸에 따라 명령이 다른 곳은 둘 다 적어 두었다.
...
| `AUTH_SECRET` | 프론트 세션 쿠키 서명 키 | **반드시 랜덤값.** 생성법은 아래 |

> 개발 환경이라 "아무 값이나"라고 썼지만, **운영 배포에서는 전부 강한 랜덤값이어야 한다.**

**랜덤값 생성** — Windows에는 `openssl`이 없으므로 PowerShell 내장 기능을 쓴다.

```powershell
# PowerShell (Windows) — AUTH_SECRET 용 32바이트 base64
$b = New-Object byte[] 32
[Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($b)
[Convert]::ToBase64String($b)

# 비밀번호처럼 16진수만 필요하면 (특수문자가 없어 안전하다)
...
openssl rand -hex 16        # 비밀번호
```

> 비밀번호에는 **16진수처럼 특수문자 없는 값**을 권한다. `$`·`/`·`+`가 들어가면 compose의 변수 보간이나 DB 접속 URL에서 깨질 수 있다.
...
```powershell
# PowerShell (Windows) — bash 스크립트라 Git Bash를 경유한다.
# 그냥 `bash`를 쓰면 WSL이 잡혀서 docker를 못 볼 수 있다. 전체 경로로 부른다.
& "C:\Program Files\Git\bin\bash.exe" ./docker/keycloak/bootstrap-dev.sh
```
...
PowerShell에는 `set -a` 같은 게 없다. `.env`를 읽어 환경변수로 올리는 줄을 **터미널마다** 실행한다.
...
> `-split '=', 2`의 **`2`가 중요하다.** 값에 `=`가 들어가는 키가 있다(`AUTH_SECRET`은 base64라 끝이 `=`로 끝난다). 이걸 빼면 값이 잘려서 세션 서명이 깨진다.
>
> 매번 붙여넣기 귀찮으면 `loadenv.ps1`로 저장해 두고 `. .\loadenv.ps1`로 부른다(앞의 `.`이 있어야 현재 셸에 반영된다).
...
`set -a; . ./.env; set +a`는 `.env`의 값들을 현재 셸의 환경변수로 만든다. **이 줄(PowerShell이면 위의 `Get-Content .env | ...` 블록)을 빼먹으면** 코어가 DB 비밀번호를 몰라 기동에 실패하거나, 프론트가 Keycloak을 찾지 못한다.

Windows에서는 `./gradlew`가 아니라 **`.\gradlew.bat`** 이다. `gradlew`(확장자 없는 것)는 셸 스크립트라 PowerShell이 실행하지 못한다.
...
다음 날 다시 시작할 때는 `docker compose start`로 인프라를 깨우고(**bootstrap을 다시 안 돌려도 된다**), 단계 6과 똑같이 터미널 두 개에서 코어와 프론트를 띄운다.
...
> # PowerShell
> docker compose up -d
> & "C:\Program Files\Git\bin\bash.exe" ./docker/keycloak/bootstrap-dev.sh    # 잊지 말 것
> ```
>
> ```bash
> # bash
...
# PowerShell (Windows)
docker compose down
Remove-Item -Recurse -Force .\data           # PostgreSQL · Elasticsearch
Remove-Item -Recurse -Force .\backend\data   # 업로드 원본 (개발에서는 여기에 쌓인다 — 위 설명 참고)
docker compose up -d
& "C:\Program Files\Git\bin\bash.exe" ./docker/keycloak/bootstrap-dev.sh
```
...
>
> **지우기 전에 백업한다.** 백업 폴더 이름을 `data-*`로 두면 `.gitignore`가 알아서 무시한다.
>
> ```powershell
> # PowerShell
> $d = Get-Date -Format 'yyyyMMdd'
> Copy-Item -Recurse .\data ".\data-backup-$d"
> Copy-Item -Recurse .\backend\data ".\backend\data-backup-$d"
> ```
>
> ```bash
> # bash
...
```powershell
# PowerShell (Windows) — 포트를 잡고 있는 프로세스를 찾아 죽인다
...
```powershell
# 한 방에: llmhub 관련 java·node를 전부 찾아 죽인다 (코어 + 프론트)
Get-CimInstance Win32_Process -Filter "Name='java.exe' or Name='node.exe'" |
  Where-Object { $_.CommandLine -like '*llmhub*' } |
...
> `gradlew`와 `next dev`는 **자식 프로세스를 띄운다.** 자식만 죽이면 부모가 포트를 다시 잡거나 좀비가 남을 수 있으니, 위의 "한 방에" 블록처럼 `llmhub` 명령줄을 가진 프로세스를 전부 잡는 편이 확실하다.
...
> **Windows(PowerShell) 사용자에게 — 아래 예제를 옮기는 규칙 두 개.**
>
> | bash | PowerShell |
> |---|---|
> | `curl` | **`curl.exe`** — PowerShell의 `curl`은 `Invoke-WebRequest` 별칭이라 `-d`·`-F`를 모른다 |
> | 줄 끝 `\` (줄바꿈) | 줄 끝 `` ` `` (백틱) |
>
> `$TOKEN` 같은 변수 표기는 양쪽이 같다. 3-1과 3-2에는 PowerShell판을 그대로 적어 두었으니, 나머지 예제는 이 규칙만 적용하면 된다.
...
```powershell
# PowerShell (Windows)
# .env 값을 읽어온다 (단계 6과 같은 블록)
Get-Content .env | Where-Object { $_ -match '^\s*[^#\s].*?=' } | ForEach-Object {
  $k, $v = $_ -split '=', 2
  [Environment]::SetEnvironmentVariable($k.Trim(), $v.Trim(), 'Process')
}

# curl 이 아니라 curl.exe 다. PowerShell의 `curl`은 Invoke-WebRequest 별칭이라 -d를 모른다.
$TOKEN = (curl.exe -s -X POST `
  ...
  -d "password=$env:DEV_ADMIN_PASSWORD" | ConvertFrom-Json).access_token

$TOKEN.Substring(0, 20) + "..."    # 값이 찍히면 성공
```
...
```powershell
# PowerShell (Windows)
curl.exe -X POST http://localhost:8080/api/index `
