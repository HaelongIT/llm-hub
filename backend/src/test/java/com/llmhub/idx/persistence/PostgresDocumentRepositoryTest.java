...
class PostgresDocumentRepositoryTest {
	...
	void 새_문서를_저장한다() {
		DocumentRecord 저장됨 =
				repository.upsert("규정-2026", "인사규정.pdf", "key-1", List.of("public"), "bge-m3", "token-v0", null);
	...
	void 같은_doc_key는_같은_id를_유지한다() {
		DocumentRecord 구버전 = repository.upsert("교체-대상", "v1.pdf", "key-old", List.of("public"), "bge-m3", "token-v0", null);

		DocumentRecord 신버전 =
				repository.upsert("교체-대상", "v2.pdf", "key-new", List.of("public", "restricted"), "bge-m3", "token-v0", null);
	...
	void 접근_태그_배열이_보존된다() {
		DocumentRecord 저장됨 =
				repository.upsert("태그-보존", "기밀.pdf", "key-2", List.of("restricted", "public"), "bge-m3", "token-v0", null);
	...
	void doc_key로_찾는다() {
		repository.upsert("조회-대상", "원본.pdf", "key-원본", List.of("public"), "bge-m3", "token-v0", null);
	...
	void 업로더가_기록된다() {
		...
		repository.upsert("업로더-기록", "a.pdf", "k1", List.of("public"), "bge-m3", "token-v0", 올린이);
	...
	void 재색인은_업로더를_유지한다() {
		...
		repository.upsert("업로더-유지", "a.pdf", "k1", List.of("public"), "옛-모델", "token-v0", 올린이);

		// 재색인은 보관된 원본을 다시 읽을 뿐이다. 올린 사람은 그대로다.
		repository.upsert("업로더-유지", "a.pdf", "k1", List.of("public"), "새-모델", "token-v0", null);
	...
	void 재업로드는_업로더를_갱신한다() {
		...
		repository.upsert("업로더-갱신", "v1.pdf", "k1", List.of("public"), "bge-m3", "token-v0", 처음_올린이);

		repository.upsert("업로더-갱신", "v2.pdf", "k2", List.of("public"), "bge-m3", "token-v0", 나중_올린이);
	...
	// docs/03: chunking_version = 색인에 쓴 청킹 버전. 값의 출처는 ChunkingStrategy다 (E11).

	@Test
	@DisplayName("재색인하면 chunking_version이 현재 전략의 버전으로 갱신된다")
	void 청킹_버전이_갱신된다() {
		repository.upsert("청킹-갱신", "a.pdf", "k1", List.of("public"), "bge-m3", "token-v0", null);

		// 청킹 전략을 바꾸고 재색인했다.
		repository.upsert("청킹-갱신", "a.pdf", "k1", List.of("public"), "bge-m3", "sentence-v1", null);

		assertThat(jpaRepository.findByDocKey("청킹-갱신").orElseThrow().getChunkingVersion())
				.as("갱신되지 않으면 재색인해도 대상 목록에서 사라지지 않는다 (E11)")
	...
	void 교체_시각이_갱신된다() throws InterruptedException {
		repository.upsert("시각-추적", "v1.pdf", "key-a", List.of("public"), "bge-m3", "token-v0", null);
		DocumentEntity 처음 = jpaRepository.findByDocKey("시각-추적").orElseThrow();

		Thread.sleep(10);
		repository.upsert("시각-추적", "v2.pdf", "key-b", List.of("public"), "bge-m3", "token-v0", null);
