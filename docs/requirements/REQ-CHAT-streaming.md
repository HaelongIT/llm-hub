...
## 테스트 시나리오 (TDD)
- [ ] 질문 → text 조각들이 순차 스트리밍되고 done으로 종료.
- [ ] 응답에 sources 이벤트가 포함되고, 그 내용이 SEARCH 결과와 일치.
- [ ] 후속 질문에 최근 N턴이 컨텍스트로 반영됨.
- [ ] LLM 장애 주입 → error 이벤트로 스트림 종료(행 없음).
...
- [ ] `ContextAssembler` 교체 시 컨트롤러 변경 없이 조립 전략만 바뀜.
