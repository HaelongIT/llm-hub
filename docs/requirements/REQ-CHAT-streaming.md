...
- [x] 스트림 완료 후 user/assistant 메시지가 저장됨(sources 스냅샷 포함).
- [x] 저장을 강제 실패시켜도 사용자는 정상 응답을 받음(응답 롤백 없음).
- [x] 블로킹 저장이 논블로킹 스레드를 점유하지 않음(스케줄러 격리 검증).
- [x] 세션 삭제 → 메시지 사라짐, 해당 대화의 감사 로그는 남음.
