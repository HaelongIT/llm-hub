...
- [x] 색인 임베딩 모델 = 설정값.
- [ ] 재색인 요청 → 보관된 원본을 다시 읽어 새 indexing_run_id로 조각이 교체된다.
- [ ] 재색인은 파일 업로드 없이 doc_key만으로 동작한다.
- [ ] 재색인 시 접근 태그는 document의 현재 값을 쓴다.
- [ ] 재색인 중 임베딩 실패 → 구버전 조각이 여전히 검색된다(증발 없음, S17×S8-3).
- [ ] 임베딩 차원이 기존 인덱스와 다르면 임베딩을 시작하지 않고 거부한다.
- [ ] 재색인 후 document.embedding_model이 현재 설정값으로 갱신된다.
- [ ] 없는 doc_key로 재색인 → 404.
- [ ] 비-ADMIN의 재색인·대상조회 시도 → 거부(403).
