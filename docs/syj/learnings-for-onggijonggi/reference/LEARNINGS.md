...
## [2026-07-12] 자잘한 하드닝 마감 — 업로드 MIME 스니핑(L-11)·KC healthcheck(L-9)
- **L-11 — 업로드 내용 스니핑은 Tika `Tika.detect(byte[])`로.** 파일명을 안 넘기면 순수 magic bytes 기반이라 확장자 편향이 없다(`detect(byte[], name)`는 이름 힌트를 섞으니 피한다). 허용목록(확장자+클라 MIME)은 그대로 1차 게이트로 두고, 스니핑은 **탐지된 타입이 실행파일/스크립트 등 위험 집합이면 거부**하는 방어심층으로 얹었다(엄격히 "탐지=허용목록"으로 하면 Tika가 못 잡는 포맷·테스트 픽스처를 과도 거부해 파장이 크다 — hwp는 octet-stream으로 잡혀 통과해야 정상). ELF 매직(`0x7F454C46`)을 pdf로 위장 → 거부로 뮤테이션 검증. `org.apache.tika.Tika`는 spring-ai-tika로 이미 classpath.
- **L-9 — KC 26 이미지엔 curl/wget이 없지만 `/bin/sh`가 bash라 `/dev/tcp`가 된다.** `KC_HEALTH_ENABLED=true`면 관리 포트 **9000**에 `/health/ready`가 뜬다(dev의 start-dev에서도 동작 — health는 모드 무관). healthcheck: `exec 3<>/dev/tcp/localhost/9000 && printf 'GET /health/ready HTTP/1.1\r\n...\r\n\r\n' >&3 && grep -q '200 OK' <&3`. YAML 이스케이프는 double-quote에서 `\r\n`(→ `\r\n` → printf가 해석). frontend `depends_on: keycloak: condition: service_healthy`로 KC 준비 후 기동. 실측: 재생성 후 ~25s에 healthy.
- **healthcheck는 컨테이너 생성 시 박히므로** 적용하려면 `up -d --force-recreate <svc>`가 필요하다. dev keycloak 재생성은 H2를 리셋하니 `bootstrap-dev.sh`를 다시 돌려 시크릿·사용자를 복원한다.

## [2026-07-12] 프론트 UI 격상 — 열람실 4가지 다듬기 (다크·슬립·진입·마크다운)
기존 방향 A(열람실)를 갈아엎지 않고 격상. CSS가 전부 `:root` 색 토큰 위에 서 있던 게 다크모드를 값싸게 만들었다.
- **토큰만 리매핑하면 다크는 거의 공짜다.** `:root[data-theme='dark']`에서 색 토큰 9개만 바꾸면 모든 규칙이 `var(--token)`이라 따라온다. 버튼이 `color:var(--paper)` on `background:var(--grounded)`였는데, 라이트/다크에서 두 토큰이 함께 뒤집혀 "밝은 배경 위 어두운 글자" 대비가 **자동으로** 유지됐다. `color-scheme`도 같이 넘겨 스크롤바·textarea 등 네이티브 컨트롤이 테마를 따르게 한다.
- **테마-FOUC + 하이드레이션 함정.** 페인트 전에 `data-theme`를 확정하려면 `<head>`의 동기 인라인 스크립트(localStorage→없으면 `matchMedia`)가 필요하다. 그러면 서버 HTML엔 없던 속성을 클라가 붙여 **하이드레이션 불일치 경고**가 뜬다 → `<html suppressHydrationWarning>`로 이 요소 속성만 무시(정석 패턴). 토글 글리프는 테마 무관 고정(◐)으로 두고 라벨(방향)만 마운트 후 확정해야 하이드레이션 안전.
...
## [2026-07-13] 게이트웨이 뒷단을 원격으로 열다 — 그리고 S8-4의 방어선이 비어 있었다는 것
사내 DGX Spark의 LiteLLM(OpenAI 호환)을 채팅 백엔드로 쓸 수 있게 배선을 일반화하다가, 임베딩 쪽 방어선이 사실상 작동하지 않고 있었음을 발견했다.

- **논리 모델명이 조각 메타에 박히는데, 그 값이 `placeholder-embedding-model`이었다.** S8-4는 "조각 메타에 모델명·차원 기록"을 요구하고 코드는 그걸 지킨다 — `EmbeddingSpec(properties.model(), ...)` → `ChunkAssembler` → ES. 그런데 기록되는 값이 **게이트웨이에 넘기는 논리 이름**이지 실제 모델이 아니다. 실제 모델(`ollama/bge-m3`)은 `LITELLM_EMBEDDING_BACKEND`가 들고 있고 코어는 그 존재를 모른다. **즉 "모델명 기록"이 아무 정보도 담고 있지 않았다.** ES 실측으로 확인: `"embedding_model":"placeholder-embedding-model"`.
- **그 결과 stale 판정에 사각지대가 있었다.** `IndexingService.staleDocuments()`는 조각의 `embedding_model`과 현재 `EMBEDDING_MODEL`을 비교한다. 둘 다 논리 이름이므로, 운영자가 **뒷단만 갈아끼우면 이름은 그대로**다 → 재색인 대상으로 안 잡힌다 → 색인 벡터와 질문 벡터가 다른 공간에 있는데 **에러도 stale도 없다.** 차원이 우연히 같으면 409 가드(`EmbeddingDimensionMismatchException`)도 안 걸린다.
- **고친 방법은 코드가 아니라 이름이다.** `EMBEDDING_MODEL`을 `bge-m3`(실제 모델)로 바꾸고 `config.yaml`의 `model_name`도 맞췄다. 이제 뒷단을 바꾸려면 이름도 바꾸게 되고, 기존 stale 판정이 되살아난다. **실증:** 이름 변경 직후 `GET /api/index/stale`이 기존 문서 5건을 전부 `reason=MODEL`로 잡았다(이전엔 0건이었다). 재색인 후 `[]`.
- **채팅과 임베딩의 이름 규칙을 일부러 비대칭으로 뒀다.** 채팅 논리명은 `chat`으로 **고정**한다 — 어디에도 저장되지 않으므로 뒷단 교체가 `.env` 세 줄로 끝난다. 임베딩 논리명은 **실제 모델을 가리켜야** 한다 — 그 이름이 **데이터 계약**이기 때문이다. 이 비대칭은 실수가 아니라 정합성 요구에서 나온 것이라 `config.yaml`에 이유를 주석으로 남겼다.
- **채팅·임베딩은 애초에 갈라져 있었다(설정만으로 분리 가능).** 채팅은 Spring AI 자동설정(`spring.ai.openai.*`, `ChatConfig`는 `ChatModel`을 주입만 받는다), 임베딩은 커스텀(`llmhub.embedding.*` + `LiteLlmEmbeddingClient`). base-url을 "공유"하는 게 아니라 **같은 환경변수를 기본값으로 참조할 뿐**이다. 그래서 Java 무수정으로 채팅만 원격에 보낼 수 있다.
- **LiteLLM의 `model=None` 오류는 모델 설정 문제가 아닐 수 있다.** `os.environ/` 배선이 멀쩡한데도 `Invalid model name passed in model=None`이 났는데, 진짜 원인은 **요청 본문이 UTF-8로 안 읽힌 것**이었다(로그에 `UnicodeDecodeError`/`orjson.JSONDecodeError`). Git Bash에서 `curl -d '{"content":"한글..."}'`을 인라인으로 주면 인코딩이 깨진다. 본문을 못 읽으니 `model` 필드도 못 읽어 None이 된 것 — **에러 메시지가 원인을 정반대로 가리킨다.** 한글 본문은 UTF-8 파일로 `--data-binary @file`로 넘긴다.
- **컨테이너 env는 compose `environment:`에만 있는 것이 전부다(R-6 재확인).** `config.yaml`이 `os.environ/X`를 읽어도 X가 compose에 나열돼 있지 않으면 컨테이너 안에 없다. 검증은 `docker compose config`가 아니라 `docker exec <c> env | grep X`가 확실하다.
