...
- [ ] **고아 조각 스위퍼** — `idx_run_id`가 `cur`도 `pending`도 아닌 조각 + 삭제된 문서의 전체 조각을
      주기적으로 정리(§5-1a). 태그 정합성 배치와 같은 배치에 얹는다.
...
| 문서 삭제 경로 설계 | **CLOSED** | §5-7 — 소프트 삭제, ES→PG→원본(유예), 자기 부서만, 재업로드 시 부활 |
| 좀비 워커 펜싱 | **CLOSED** | §5-8 — 조건부 UPDATE + §5-1a 스위퍼가 ES 잔여물 회수 |
| 부서 개편 시 운영 대응 | **CLOSED** | §5-3 — 운영자가 DB에서 `department` 직접 수정(수동 절차) |
...
| **파라미터 값들** | **OPEN** | 운영 정책에 달린 숫자라 설계로 정하지 않았다: 리퍼 타임아웃 · 정합성 배치 주기 · `attempt_cnt` 상한 · 원본 유예 기간(N일) |
...
> 배치는 문서마다 다음을 삭제한다.
> 1. `idx_run_id`가 **`cur_idx_run_id`도 `pending_idx_run_id`도 아닌** 조각 — 버려진 런의 잔여물.
> 2. `deleted_at IS NOT NULL`인 문서의 **모든** 조각 — 삭제 실패분.
...
- **삭제 순서**: llmhub 자신의 `OPEN-QUESTIONS.md`가 이미 이 순서를 정해 뒀다 — **ES 조각 → PG →
  ...
  3. 보관 원본 파일은 **바로 지우지 않는다 — (7라운드 확정, 사용자 확인)** N일 유예 후 별도 배치가
     삭제한다. 유예 기간 안에는 보관 원본으로 재색인해 **복구할 수 있다.** 즉시 지우면 ES 조각·원본이
     모두 사라져 소프트 삭제가 순수 감사용으로만 남는데, "잘못 올린 문서 회수"가 삭제의 주요 동기였던
     만큼 "잘못 지운 문서"도 같은 무게로 다뤄야 한다. 유예 기간(N일)은 파라미터.
...
- **삭제는 `status`를 건드리지 않는다 — (7라운드 추가.)** 색인 수명주기(`PENDING`→`PROCESSING`→
  ...
  `deleted_at IS NULL`을 기본 조건으로 하므로(§5-1a) 아무도 그 행을 건드리지 않고, 부활하면 어차피
  아래 목록대로 초기화된다.
- **검색 쿼리에서 `deleted_at`을 체크하지 않는다 — 단 (7라운드 정정) 근거가 반대다.** 원래
  ...
  **체크할 방법이 애초에 없고**, 그렇기 때문에 **ES 조각 삭제가 반드시 백스톱돼야 한다**(§5-1a
  스위퍼 2번 규칙). 결론(검색 쿼리는 안 고친다)은 같지만, 그 결론이 성립하려면 백스톱이 필수라는
  게 핵심이다.
...
**(7라운드 추가) 부활 시 초기화 목록 — 명시가 빠져 있었다.** 삭제 1단계가 ES 조각을 **전부** 지웠으므로,
`deleted_at`만 되돌리면 나머지 필드가 존재하지 않는 조각을 가리키며 거짓말을 한다. 부활은 **"완전히 새
문서처럼"** 시작해야 한다.
...
| `attempt_cnt` | `0` | 부활은 **재업로드의 특수 케이스**이므로 §5-2의 리셋 규칙이 그대로 적용된다 |
...
- 2절·5절의 설계 제안은 **llmhub 코드로 검증된 사실이 아니다.** llmhub엔 비동기 워커·부서 스코프·
  ...
  구분해서 읽을 것.
- 이 문서는 이 시점(7라운드)까지의 논의 스냅샷이다. 후속 라운드 결과는 이 파일에 계속 이어 붙일 것 —
