...
- **(5라운드 추가) 문서 삭제 경로 설계** — 4라운드에 걸쳐 한 번도 다루지 않았다. llmhub도 v0에서
  명시적으로 미룬 전례가 있어(`OPEN-QUESTIONS.md`) 지금 없는 것 자체는 이상하지 않지만, 필요성은
  이미 두 곳에서 나왔다: 잘못 올린 문서 회수, 그리고 §5-3 부서 개편 대응의 잠재적 대안 경로(재업로드
  + 구 문서 삭제). 다음 라운드에 정식 의제로 올린다.
- **(5라운드 추가) 좀비 워커 펜싱** — §5-2에서 채택한 "FAILED 확정 시 즉시 조각 삭제"가, 타임아웃으로
  "죽었다" 판정했지만 실제로는 느릴 뿐 살아있던 워커와 경합할 수 있다(그 워커가 뒤늦게 쓰는 조각과
  충돌). 펜싱 토큰 같은 정석 해법이 있으나 지금 결론을 낼 필요는 없어 다음 라운드로 남긴다.
...
- 2절·5절의 설계 제안은 **llmhub 코드로 검증된 사실이 아니다.** llmhub엔 비동기 워커·부서 스코프·
  ...
  구분해서 읽을 것.
- 이 문서는 이 시점(5라운드)까지의 논의 스냅샷이다. 후속 라운드 결과는 이 파일에 계속 이어 붙일 것 —
