...
- [ ] `document.acc_tag_ver`(정수) + 조각에 복사 — §5-1. 정합성 검사(`acc_tag_ver` 불일치 조각 재갱신)
      배치도 같이 필요.
- [ ] `document.deleted_at`, `deleted_by`(FK, `ON DELETE SET NULL`) — §5-7 소프트 삭제.
...
- ~~문서 삭제 경로 설계~~ — **(6라운드에서 해결)** §5-7 참고(소프트 삭제, ES→PG→원본 순서, 재업로드
  시 부활).
- ~~좀비 워커 펜싱~~ — **(6라운드에서 해결)** §5-8 참고(최종 커밋 직전 3조건 재검증으로 삭제-중-색인
  경합과 함께 해결).
...
- **부분 색인 조각의 수명 — (5라운드 정정) 자가 치유는 "다음 성공한 런"에서만 일어난다는 게 빠져
  ...
  지목할 수 있다. (잔여 리스크: 타임아웃으로 "죽었다" 판정했는데 워커가 실제로는 느릴 뿐 살아있던
  경우 — 좀비 워커가 뒤늦게 쓰는 조각과 충돌할 수 있다. §4로 남긴다.)
...
- **`document.deleted_at`**(nullable timestamptz), **`document.deleted_by`**(nullable, `app_user` FK,
  `ON DELETE SET NULL` — `uploaded_by`와 같은 이유, V4 참고) 추가.
- **삭제 순서**: llmhub 자신의 `OPEN-QUESTIONS.md`가 이미 이 순서를 정해 뒀다 — **ES 조각 → PG →
  ...
  2. `document.deleted_at`+`deleted_by` 기록(PG 행 자체는 유지 — 소프트 삭제).
  3. 보관 원본 파일 삭제.
...
- **검색 필터링은 별도로 필요 없다.** ES 조각을 1단계에서 이미 하드 삭제했으므로, 삭제된 문서는
  검색에 아예 안 걸린다 — `deleted_at`을 검색 쿼리에서 따로 체크할 필요가 없다.

**설계 중 발견한 추가 함정 — 재업로드 시 PK 충돌.** `document.id`는 `(department, doc_key)`에서
결정적으로 유도된다(§2-3). 소프트 삭제는 PG 행을 지우지 않으므로, **같은 `doc_key`를 나중에
다시 올리면 똑같은 id가 유도되어 기존(소프트 삭제된) 행과 기본키가 충돌한다.** 해결: **재업로드 시
기존 소프트 삭제 행을 "부활"시킨다** — `deleted_at`/`deleted_by`를 `NULL`로 되돌리고 나머지 필드를
새 업로드 내용으로 갱신한다. 이건 새 개념이 아니라 S17의 upsert-by-doc_key(재업로드 = 기존 행 갱신)
원칙이 소프트 삭제까지 자연스럽게 확장된 것뿐이다.
...
살아있는 행끼리만 유니크를 강제하고, 삭제된 행은 제약에서 빠진다(어차피 재업로드는 삭제 행을
부활시키므로 삭제된 상태에서 같은 키로 새 행을 만드는 경우 자체가 없다).

### 5-8. 좀비 워커 펜싱 — (6라운드) 통합 재검증으로 해결

§5-2(잔여 리스크로 남겨둠)와 §5-7(진행 중 삭제 요청)이 **같은 문제**를 공유한다 — 워커가 마지막
커밋을 하려는 시점에, 그사이 다른 주체(리퍼·삭제 요청)가 이 런을 이미 무효로 만들었을 수 있다.
하나의 메커니즘으로 묶는다.

**채택하는 방식**: 워커가 **최종 PG 커밋 직전**(`READY`/`FAILED` 확정 직전) 문서 행을 다시 읽어
다음 세 조건을 재확인한다.
1. `status == PROCESSING`(리퍼가 이미 `FAILED`로 회수하지 않았는지)
2. `pending_idx_run_id == 자신의 run id`(다른 런으로 대체되지 않았는지)
3. `deleted_at IS NULL`(그사이 삭제되지 않았는지)

**셋 중 하나라도 깨지면 워커는 자기 결과를 커밋하지 않고 즉시 중단한다** — 이미 쓴 ES 조각이 있으면
지운다(가능한 범위에서). 이 재검증 지점 하나가 좀비 워커 문제와 삭제-중-색인 경합 문제를 동시에
막는다 — 별도 장치 두 개를 만들 필요가 없다.
...
- 2절·5절의 설계 제안은 **llmhub 코드로 검증된 사실이 아니다.** llmhub엔 비동기 워커·부서 스코프·
  ...
  구분해서 읽을 것.
- 이 문서는 이 시점(6라운드)까지의 논의 스냅샷이다. 후속 라운드 결과는 이 파일에 계속 이어 붙일 것 —
